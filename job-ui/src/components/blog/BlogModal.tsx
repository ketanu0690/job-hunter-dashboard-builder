...
const BlogModal: React.FC<BlogModalProps> = ({ open, onClose, blog, source }) => {
  if (!blog) return null;
            ...
            <div>
              <DialogHeader className="p-0">
                ...
                <DialogDescription className="mt-2 flex items-center gap-2 text-sm">
                  <span className={isMedium ? "text-indigo-600 dark:text-indigo-400" : "text-emerald-600 dark:text-emerald-400"}>
                    {source}
                  </span>
                  {blog.pubDate && (
                    <>
                      <span className="text-muted-foreground">•</span>
                      <span className="text-muted-foreground">{formatDate(blog.pubDate)}</span>
                    </>
                  )}
                </DialogDescription>
              </DialogHeader>
