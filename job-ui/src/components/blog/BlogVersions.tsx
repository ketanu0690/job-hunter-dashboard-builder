            ...
            versions.map((version, index) => (
              <Card key={version.versionId || index} className="glass-card">
                ...
                <CardContent className="py-2 px-4">
                  <p className="text-sm font-medium mb-1 truncate">{version.title}</p>
                  <p className="text-xs text-muted-foreground line-clamp-2 mb-3">{version.excerpt || version.content.substring(0, 100)}</p>
                  <Button 
                    size="sm"
                    variant="outline"
                    className="w-full"
