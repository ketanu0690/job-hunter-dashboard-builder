  ...
  const processExcel = (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const requiredFields = ['title', 'company', 'location', 'description', 'url', 'date'];
      ...
      reader.onload = async (e) => {
        try {
          ...
          const processedJobs = jsonData.map((job: any) => ({
            ...
            skills: job.skills ? 
              (typeof job.skills === 'string' ? job.skills.split(',').map((s: string) => s.trim()) : job.skills) 
              : [],
  ...
  return (
    <div className="space-y-4 p-4 border rounded-lg bg-white dark:bg-gray-800">
      ...
      <div className="grid gap-4">
        <label className="block">
          ...
          <div className="mt-1 flex items-center gap-4">
            ...
            <span className="text-sm text-gray-500 dark:text-gray-400">
              {preview ? `File selected (${preview.total} jobs)` : 'No file selected'}
            </span>
          </div>
        </label>
        ...
        {preview && (
          <div className="mt-4">
            <h4 className="font-medium mb-2">Preview (showing 3 of {preview.total} jobs):</h4>
            <div className="bg-gray-50 dark:bg-gray-900 p-3 rounded overflow-auto max-h-60">
              <pre className="text-xs">{JSON.stringify(preview.sample, null, 2)}</pre>
            </div>
