...
class LinkedinEasyApply {
  ...
  private coverLetterDir: string;
  private checkboxes: Record<string, any>;
  private universityGpa: number;
  private languages: Record<string, string>;
  private industry: Record<string, number>;
  private technology: Record<string, number>;
  private personalInfo: Record<string, string>;
  private eeo: Record<string, string>;
  private technologyDefault: number;
  private industryDefault: number;
  private browser: any;
  private page: any;
  ...
  async login() {
    try {
      ...
      await require('util').promisify(setTimeout)(require('random').int(5000, 10000));
  ...
  async scrollSlow(scrollableElement: any, options: ScrollOptions = {}) {
    ...
    while (condition()) {
      await scrollableElement.evaluate((el: any, pos: number) => {
        el.scrollTo(0, pos);
      }, current);
      
      await require('util').promisify(setTimeout)(require('random').int(1000, 2600));
      current += increment;
    }
  }
  ...
  getBaseSearchUrl(parameters: any) {
    let remoteUrl = "";
  ...
  async nextJobPage(position: string, location: string, jobPage: number) {
  ...
  // Helper functions
  cartesianProduct(a: string[], b: string[]) {
...
module.exports = LinkedinEasyApply;
