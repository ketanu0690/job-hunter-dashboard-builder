...
const EasyApplyForm: React.FC<EasyApplyFormProps> = ({ platform, onClose }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const yamlContent = yaml.dump(configData);
      await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/save-config`, { yaml: yamlContent });
  ...
  const messageColor = message === 'Configuration saved successfully!' ? 'text-green-500' : 'text-red-500';

  if (!platform) return null;

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-900 rounded-lg p-6 w-full max-w-2xl space-y-4">
